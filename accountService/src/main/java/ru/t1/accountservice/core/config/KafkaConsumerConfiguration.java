...
public class KafkaConsumerConfig {
    ...
    @Bean
    ConsumerFactory<String, TransactionCreateRequest> transactionsCreateConsumerFactory() {
        Map<String, Object> config = new HashMap<>();
        config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, servers);
        config.put(ConsumerConfig.GROUP_ID_CONFIG, transactionsGroupId);
        config.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        config.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        config.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "ru.t1.accountservice.api.dto.transaction.TransactionCreateRequest");
        ...
        //        return factory;    return new DefaultKafkaProducerFactory<>(config);
    }

    @Bean
    ConcurrentKafkaListenerContainerFactory<String, TransactionCreateRequest> kafkaListenerTransactionsCreateContainerFactory(@Qualifier("transactionsCreateConsumerFactory") ConsumerFactory<String, TransactionCreateRequest> transactionsCreateConsumerFactory) {
        ConcurrentKafkaListenerContainerFactory<String, TransactionCreateRequest> factory = new ConcurrentKafkaListenerContainerFactory<>();
